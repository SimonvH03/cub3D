...
/*   Updated: 2025/04/22 18:17:50 by simon         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
		...
		const char *path)
{
	...
	if (access(path, F_OK) != 0)
		return (set_error(CUB_TEXPATH));
	if (ft_strnstr(path, ".png", -1) != NULL)
	{
		*dest = mlx_load_png(path);
		if (*dest == NULL)
			return (set_error(CUB_MLXFAIL));
	}
	else if (ft_strnstr(path, ".xpm42", -1) != NULL)
	{
		*dest = modlx_load_xpm42(path);
		if (*dest == NULL)
		...
		const char *line)
{
	...
	if (alpha < 0 || alpha > 255
		...
		|| green < 0 || green > 255
		|| blue < 0 || blue > 255)
		return (set_error(CUB_CLRFORM));
	*dest = (uint32_t)(((char)alpha << 24)
			| ((char)blue << 16)
			| ((char)green << 8)
			| (char)red);
	return (RETURN_SUCCESS);
