...
/*   Updated: 2025/05/05 22:26:31 by simon         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
		...
		t_walls *walls)
{
	...
	column->tex_y = 0;
	column->start = walls->image->height / 2 - column->half_height;
	if (column->start < 0)
	{
		column->tex_y = -column->start * column->step;
		column->start = 0;
	}
	column->end = walls->image->height / 2 + column->half_height;
		...
		uint32_t screen_x)
{
	...
	vertical_boundaries(&column, ray, walls);
